...
class NDEFGui:
	...
	def uri_write(self, uri):
		...
		if self.outfile != None:
			tag = NFCTagType1.NFCTagType1()
			tag.setNDEF(u.getNDEFRecord().getHex())
			fp = open(self.outfile, 'wb')
			fp.write(tag.getTag(0))
			fp.close()
		else:
...
if __name__ == "__main__":
	if len(os.sys.argv) < 2:
		print "NDEFGuiF Copyright: Collin Mulliner (http://www.mulliner.org/nfc/)"
		print "License: GPLv2"
		print " syntax: " + os.sys.argv[0] + " <out file>"
		print " write tag using: ndef_mifare -w <out file>"
	else:
		main = NDEFGui(0,0, os.sys.argv[1])
		gtk.main()
